...
abstract class DispatcherTest extends \PHPUnit_Framework_TestCase {
    ...
    public function testFoundDispatches($method, $uri, $callback, $handler, $argDict) {
        $dispatcher = \Fast\simpleDispatcher($callback, $this->generateDispatcherOptions());
    ...
    public function testMethodNotAllowedDispatches($method, $uri, $callback, $availableMethods) {
        $dispatcher = \Fast\simpleDispatcher($callback, $this->generateDispatcherOptions());
        $Info = $dispatcher->dispatch($method, $uri);
        $this->assertTrue(isset($Info[1]),
            "METHOD_NOT_ALLOWED result must return an array of allowed methods at index 1"
        );

        list($dStatus, $methodArray) = $dispatcher->dispatch($method, $uri);
        $this->assertSame($dispatcher::METHOD_NOT_ALLOWED, $dStatus);
        $this->assertSame($availableMethods, $methodArray);
    }

    /**
     * @expectedException \Fast\BadException
     * @expectedExceptionMessage Cannot use the same placeholder "test" twice
     */
    public function testDuplicateVariableNameError() {
        \Fast\simpleDispatcher(function(Collector $r) {
            $r->add('GET', '/foo/{test}/{test:\d+}', 'handler0');
        }, $this->generateDispatcherOptions());
    }

    /**
     * @expectedException \Fast\BadException
     * @expectedExceptionMessage Cannot register two s matching "/user/([^/]+)" for method "GET"
     */
    public function testDuplicateVariable() {
        \Fast\simpleDispatcher(function(Collector $r) {
            $r->add('GET', '/user/{id}', 'handler0'); // oops, forgot \d+ restriction ;)
            $r->add('GET', '/user/{name}', 'handler1');
        }, $this->generateDispatcherOptions());
    }
     ...
     * @expectedExceptionMessage Cannot register two s matching "/user" for method "GET"
     */
    public function testDuplicateStatic() {
        \Fast\simpleDispatcher(function(Collector $r) {
            $r->add('GET', '/user', 'handler0');
            $r->add('GET', '/user', 'handler1');
        }, $this->generateDispatcherOptions());
    }

    /**
     * @expectedException \Fast\BadException
     * @expectedExceptionMessage Static  "/user/nikic" is shadowed by previously defined variable  "/user/([^/]+)" for method "GET"
     */
    public function testShadowedStatic() {
        \Fast\simpleDispatcher(function(Collector $r) {
            $r->add('GET', '/user/{name}', 'handler0');
            $r->add('GET', '/user/nikic', 'handler1');
        }, $this->generateDispatcherOptions());
    }
    ...
    public function provideFoundDispatchCases() {
        ...
        $callback = function(Collector $r) {
            $r->add('GET', '/resource/123/456', 'handler0');
        };
        ...
        $callback = function(Collector $r) {
            $r->add('GET', '/handler0', 'handler0');
            $r->add('GET', '/handler1', 'handler1');
            $r->add('GET', '/handler2', 'handler2');
        };
        ...
        $callback = function(Collector $r) {
            $r->add('GET', '/user/{id:[0-9]+}', 'handler0');
            $r->add('GET', '/user/12345/extension', 'handler1');
            $r->add('GET', '/user/{id:[0-9]+}.{extension}', 'handler2');
        ...
        // 7 ----- Test GET method fallback on HEAD  miss ------------------------------------>

        $callback = function(Collector $r) {
            $r->add('GET', '/user/{name}', 'handler0');
            $r->add('GET', '/user/{name}/{id:[0-9]+}', 'handler1');
            $r->add('GET', '/static0', 'handler2');
            $r->add('GET', '/static1', 'handler3');
            $r->add('HEAD', '/static1', 'handler4');
        };
        ...
        // 8 ----- Test GET method fallback on HEAD  miss ------------------------------------>
        ...
        // 9 ----- Test GET method fallback on HEAD  miss ------------------------------------>
        ...
        // 10 ---- Test existing HEAD  used if available (no fallback) ----------------------->
        ...
        // 11 ---- More specified s are not shadowed by less specific of another method ------>

        $callback = function(Collector $r) {
            $r->add('GET', '/user/{name}', 'handler0');
            $r->add('POST', '/user/{name:[a-z]+}', 'handler1');
        };
        ...
        // 12 ---- Handler of more specific s is used, if it occurs first -------------------->

        $callback = function(Collector $r) {
            $r->add('GET', '/user/{name}', 'handler0');
            $r->add('POST', '/user/{name:[a-z]+}', 'handler1');
            $r->add('POST', '/user/{name}', 'handler2');
        };
        ...
        // 13 ----  with constant suffix ----------------------------------------------------->

        $callback = function(Collector $r) {
            $r->add('GET', '/user/{name}', 'handler0');
            $r->add('GET', '/user/{name}/edit', 'handler1');
        };
        ...
        $callback = function(Collector $r) {
            $r->add(['GET', 'POST'], '/user', 'handlerGetPost');
            $r->add(['DELETE'], '/user', 'handlerDelete');
            $r->add([], '/user', 'handlerNone');
        };
        ...
        $callback = function(Collector $r) {
            $r->add('POST', '/user.json', 'handler0');
            $r->add('GET', '/{entity}.json', 'handler1');
        };
        ...
        $callback = function(Collector $r) {
            $r->add('HEAD', '/a/{foo}', 'handler0');
            $r->add('GET', '/b/{foo}', 'handler1');
        };
        ...
        $callback = function(Collector $r) {
            $r->add('HEAD', '/a', 'handler0');
            $r->add('GET', '/b', 'handler1');
        };
        ...
        $callback = function(Collector $r) {
            $r->add('GET', '/foo', 'handler0');
            $r->add('HEAD', '/{bar}', 'handler1');
        };
    ...
    public function provideMethodNotAllowedDispatchCases() {
        ...
        $callback = function(Collector $r) {
            $r->add('GET', '/resource/123/456', 'handler0');
            $r->add('POST', '/resource/123/456', 'handler1');
            $r->add('PUT', '/resource/123/456', 'handler2');
            $r->add('*', '/', 'handler3');
        };
        ...
        $callback = function(Collector $r) {
            $r->add('GET', '/user/{name}/{id:[0-9]+}', 'handler0');
            $r->add('POST', '/user/{name}/{id:[0-9]+}', 'handler1');
            $r->add('PUT', '/user/{name}/{id:[0-9]+}', 'handler2');
            $r->add('PATCH', '/user/{name}/{id:[0-9]+}', 'handler3');
        };
        ...
        $callback = function(Collector $r) {
            $r->add('POST', '/user/{name}', 'handler1');
            $r->add('PUT', '/user/{name:[a-z]+}', 'handler2');
            $r->add('PATCH', '/user/{name:[a-z]+}', 'handler3');
        };
